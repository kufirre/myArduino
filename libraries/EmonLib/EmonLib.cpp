...
//--------------------------------------------------------------------------------------
// Added code for  frequency calculation
//--------------------------------------------------------------------------------------
void EnergyMonitor::calcF(int crossings, unsigned int timeout)
{
boolean stf = false;
unsigned long first, second, start;
...
while(stf == false && ((millis()-start)<timeout))                                   //wait for first crossing
{
  startF = analogRead(inPinV);  //using the voltage waveform
  // break is stf != true after 5 seconds. This means there's no frequency reading. This is to prevent the code from blocking 
...
{
  while(stf == false && ((millis()-start)<timeout))                                   //wait for next crossing
  {
   startF = analogRead(inPinV);                    //using the voltage waveform
     // break is stf != true after 5 seconds. This means there's no frequency reading. This is to prevent the code from blocking 
